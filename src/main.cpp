...
using namespace std;
std::piecewise_linear_distribution<double> destroy_distribution(double min, double peak, double max){
	std::array<double, 3> i{ min, peak, max };
	std::array<double, 3> w{ 1.0, 0.65, 0.25 };
	return std::piecewise_linear_distribution<double>{i.begin(), i.end(), w.begin()};
}

std::piecewise_linear_distribution<double> repair_distribution(double min, double peak, double max) {
	std::array<double, 3> i{ min, peak, max };
	std::array<double, 3> w{ 1.0, 0.5, 0.01 };
	return std::piecewise_linear_distribution<double>{i.begin(), i.end(), w.begin()};
}
...
inline int partition(int*& index, int*& dist,  int low, int high){
...
inline void quickSort(int*& index, int*& dist, int low, int high) {
	if (low < high) {
...
inline int make_feasible(vector<int>& cities, const int b,  const int n, const bool initial, vector<int>& schools, const int N_c, vector<int>& tf_schools, const int N_tf, const bool trans_b) {
	...
	if (S > 1 && cap_improv) {
		...
		int cap = 0;
		for (int y = 0; y < N_s - 1; y++) {
			if (cities[y] < P)cap += (3 * W_ch[cities[y]] + 1); // total current capacity
			else {
				for (int z = y - 1; z >= 0; z--) {
					if (cities[z] < P && St_Sc[cities[z]] == cities[y]) cap -= (3 * W_ch[cities[z]] + 1);
				}
				if (cities[y] == T) {
					for (int x = 0; x < P; x++) {
						if (ysol[x][1] == b) { //vehicle n_v is the second transfer 
							cap += (3 * W_ch[x] + 1);
						}
					}
				}
		...
		if (cap > C_b) return INT32_MAX; // then its not feasible
	}
	for (i = 0; i < N_s; i++) { // delete schools in te route first;
		if (cities[i] >= P) {
			cities.erase(cities.begin() + i);
...
inline double isFEAS(vector<vector<int>>& b_xsol, vector<vector<int>>& b_Dsol) {
	...
	second_cost = 0;
	for (b = 0; b < B; b++) {
		second_cost += (scosts[b]/b_xsol[b].size());
	}
	...
	if (curr_cap + (3 * W_ch[p] + 1) <= C_b) {
		...
		// if vehice n_v not part of the route --> see if transfer is possible 
			...
			// if not feasible  --> try to improve route first
				...
				else if (curr_capT + (3 * W_ch[p] + 1) <= C_b && i_t != -1) {
					b_trans = transfer_possible(Dsol, xsol, n_v, p, b, b2);
					if (b_trans != -1) {
						//cout << " --------------------------------------------> new vehicle accepted with TRANSFER v_" << b_trans << endl;
						ysol[p][0] = n_v; // assign vehicle to student
...
inline bool repair_solution2(const int b, const int b2, const int p, const int n_v, const int s, vector<vector<int>>& xsol, vector<vector<int>>& Dsol) {
	int curr_capT = 0; // current cap before transfer
	int l = -1, x, z, y, N_imp = 5000;
	bool I_S = false, initial = false;
	...
	improve_route(N_r, xsol[n_v], N_imp, I_S, n_v, initial);
	if (curr_cap + (3 * W_ch[p] + 1) <= C_b) {
		// if vehicle n_v already visits school of p --> insert and try to see if its feasible
		if (l != -1) {
			//cout << " S_" << St_Sc[p] - P << " already exisits in v_" << n_v << endl;
...
inline int printpluscost(const vector<vector<int>>& b_xsol, const vector<vector<int>>& b_Dsol, int ** ysol, bool print, bool write, string region, string tt_type) {
	...
	ofstream sol_p("data/output/" + region + "/" + tt_type + "/formal_solution.txt");
...
int main() {
	...
	// Early arrival times
	ifstream filea("data/input/" + region + "/e_arrivals.txt");
	i = 0;
	while (i < S) {
		filea >> t_ea[i];
		i++;
	...
	// Early arrival times
	ifstream fileal("data/input/" + region + "/l_arrivals.txt");
	i = 0;
	while (i < S) {
		fileal >> t_la[i];
		i++;
	...
	// Wheelchair use
	ifstream filew("data/input/" + region + "/wheelchair.txt");
	i = 0;
	while (i < P) {
		filew >> W_ch[i];
		i++;
	...
	// Assigned schools
	ifstream filesa("data/input/" + region + "/School_assignment.txt");
	i = 0;
	while (i < P) {
		filesa >> St_Sc[i];
	...
	// read locations
	double** coordinates = new double* [P];
	for (i = 0; i < P; i++) {
		coordinates[i] = new double[2];
	}
	ifstream filep("data/input/" + region + "/locations_" + region + ".txt");
	int i0 = 0;
	while (i0 < P) {
		filep >> coordinates[i0][0] >> coordinates[i0][1]; // extracts 2 floating point values seperated by whitespace
		i0++;
	}
	filep.close();

	string path = "data/input/" + region + "/travel_time_matrix_" + tt_type + ".csv";

	read_travel_matrix(path, TT);
	
	// -------------------------------------------------- VARIABLES ------------------------------------------------------------------------
	vector<vector<int>> xk; //routes of each vehicle b
	vector<vector<int>> b_xk;
	vector<vector<int>> xsol;

	vector<vector<int>> Dk; // timetable of each vehicle b
	vector<vector<int>> b_Dk;
	...
	if (BigRoute_OPT) {
		...
		for (int z = 0; z < 3; z++) {
			cout << types[z] << endl;
			path = "data/input/" + region + "/travel_time_matrix_" + types[z] + ".csv";
			...
			ofstream BR_p("data/input/" + region + "/BR_" + types[z] + ".txt");
			for (i = 0; i < L; i++) {
		...
		// read optimized
		ifstream BR_r("data/input/" + region + "/BR_" + tt_type + ".txt");
		i = 0;
		while (i < L) {
			BR_r >> best_route[i];
			i++;
					...
					if (curr_cap + (3 * W_ch[p] + 1) <= C_b) { // add as many students of the same school to a vehicle (consider capacity + max travel time)
						...
						if (insert_location(xsol, p, vehicle, Dsol)) { // add passenger location to the route in the best possible place
							...
							if (insert_location(xsol, p, vehicle, Dsol)) { // add passenger location to the route in the best possible place
								...
								//cout << "bus full cap " << curr_cap << endl;
								//N_b = xsol[vehicle].size();
								//improve_route(N_b, xsol[vehicle], N_improv*4, I_S, vehicle, initial);
								break;
							}
						}
					}
	...
	double scost_current = isFEAS(xsol, Dsol);
	if (scost_current == -1) {
	...
	int b2 = 0;
	piecewise_linear_distribution<double> des_st = destroy_distribution(0, C_b, P - 1);
	piecewise_linear_distribution<double> rep_st = repair_distribution(0, 20, L-2);
	normal_distribution<double> new_assign(0.0, 20); // used to be 0, 10
	...
	int B_current = xsol.size(), B_new = 0;
	...
	double lam = 500;
	int sig = 0;
	double scost_best = scost_current, scost_new = 0;
	while (itr < N_it) { // enter loop

		if ((itr + 1) % 5000 == 0)cout << "  ******************************************* Interation " << itr + 1 << "\tvehicles: " << B_current << "\tmax travel time: " << scost_best / B_current / 60.0 << " min/vehicle" << endl;
		xsol.clear();
		Dsol.clear();
		for (b = 0; b < B_current; b++) {
			x1.clear();
		...
		*/
		...
		if (scost_new != -1) {
			...
			if (B_new < B_current) {
				cout << " --->  Solution with less vehicles: " << B_new << ", max tt : " << scost_new / 60.0 << " min-- > on iter : " << itr + 1 << endl;
				// accept new solution
				B_current = B_new;
				scost_current = scost_new;
				if (scost_best > scost_new) scost_best = scost_new;
				b_xk.clear();
				b_Dk.clear();
				xk.clear();
				Dk.clear();
				...
				if (dE < 0) {
					...
					//accept new solution
					//cout << " solution with LUCKY max tt: " << scost_new / 60.0 << " min   -- > on iter : " << itr + 1 << endl;
					scost_current = scost_new;
					if (scost_best > scost_new) scost_best = scost_new;
					xk.clear();
					Dk.clear();
		...
		/*
			...
			//printpluscost(xsol, Dsol, ysol, TT, tau_1, tau_2, tau_3, tau_4, P, S, D_t, C_b, t_ea, t_la, W_ch, St_Sc, true, FEAS, T);
	...
	for (b = 0; b < B_current; b++) {
		N_r = b_xk[b].size();
		improve_route(N_r, b_xk[b], N_imp, I_S, b, initial);
		N_sc = 0;
		schools.clear();
		for (i = 0; i < N_r; i++) {
			if (b_xk[b][i] >= P) {
				N_sc++;
				schools.push_back(b_xk[b][i]);
			}
		}
		int* alights = new int[N_sc];
		for (int s = 0; s < N_sc; s++) {
			...
			for (int i = 0; i < N_r; i++) {
				if (b_xk[b][i] < P) {
					if (St_Sc[b_xk[b][i]] == schools[s] || (schools[s] == T && b_yk[b_xk[b][i]][1] != -1)) {
						alight += tau_3 * (1 - W_ch[b_xk[b][i]]) + tau_4 * W_ch[b_xk[b][i]];
					}
				}
			}
			if (schools[s] == T) { // boarding time
		...
		//calculate the timetable
		vector<int> time;
		bool tfeas = time_table2(b_xk[b], b_Dk[b], b, alights, N_sc);
		delete alights;
	}
	//*/
	//scost_new = isFEAS(b_xk, b_Dk); 
	scost_new==0;
	if (scost_new != -1) {
		print = true, write = true;
		if ((region == "Diest" && B_current > 18) || (region == "Gavere" && B_current > 9)) write = false;
		printpluscost(b_xk, b_Dk, b_yk, print, write, region, tt_type);

		if (write) {
			// write to file
			ofstream yk_p("data/output/" + region + "/" + tt_type + "/ysol.txt");
			ofstream xk_p("data/output/" + region + "/" + tt_type + "/xsol.txt");
			ofstream Dk_p("data/output/" + region + "/" + tt_type + "/Dsol.txt");
			for (i = 0; i < P; i++) {
				yk_p << b_yk[i][0] << "\t" << b_yk[i][1] << endl;
			}
			yk_p.close();
			int B = xk.size();
			for (b = 0; b < B; b++) {
		...
		cout << "BAD route optimization ruimed it\n";
	}
