...
int main() {
	...
	int b2 = 0;
	piecewise_linear_distribution<double> des_st = triangular_distribution(0, P / 2, P - 1);
	normal_distribution<double> new_assign(0.0, 8.0); // used to 0-10
	vector<int> old_xsol0, old_xsol1, old_xsol2, old_Dsol0, old_Dsol1, old_Dsol2;
	N_it = 15000;

	int n_des = int(P * 0.04); // used to be 30 for Diest
	...
	while (itr < N_it) { // enter loop
		if ((itr + 1) % 500 == 0)cout << "  ******************************************* Interation " << itr + 1 << "\tvehicles: " << B_current << "\tmax travel time: " << scost_current / B_current / 60.0 << " min/vehicle" << endl;
		for (i = 0; i < n_des; i++) {
			destroyed[i] = -1;
		}
		...
		for (i = 0; i < n_des; i++) {
			...
			while (!create) {
				n_v = (n_v + 1) % B_current;
				//cout << " try new n_v" << n_v << endl;
				create = repair_solution(b, b2, p, n_v, s, xsol, Dsol);
				c_it++;
				if (c_it >= B_current - 1) break;
			}
			if (!create) break;
		}
		...
		if (scost_new != -1) {
			//cout << " new solution\n";
			if (B_new < B_current) {
				cout << " solution with less vehicles: " << B_new << "   --> on iter : " << itr + 1 << endl;
				// accept new solution
				B_current = B_new;
